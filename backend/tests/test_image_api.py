...
def test_reference_image_key_uses_postcard_prefix(monkeypatch):
    ...
    # 默认 index=0（兼容老调用），多图按 index 区分
    assert reference_image_key(14, "pc-test") == "postcards/14/pc-test/reference_0.webp"
    assert reference_image_key(14, "pc-test", 2) == "postcards/14/pc-test/reference_2.webp"
