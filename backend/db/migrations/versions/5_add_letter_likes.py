...
def upgrade() -> None:
    ...
    # 某些历史部署已经创建过表但没有推进 alembic_version；迁移应可安全重试。
    if inspector.has_table("letter_likes"):
        existing_indexes = {item["name"] for item in inspector.get_indexes("letter_likes")}
        if "ix_letter_likes_user" not in existing_indexes:
