...
5. **core_place**：信件中最核心的地点名称。如果用户明确写出了地名就用它；如果信中没提但给了 place_hint 就用 place_hint；如果都没有则用城市名
6. **image_prompt**：根据前面对信件场景和情感的分析，写一段英文图像生成提示词（50-80词）。必须是 RETRO 16-BIT PIXEL ART 风格。
   关键要求：
...
class LetterAnalysisService:
    ...
    def analyze_letter_deep(
        ...
        if hometown:
            ...
            if city:
                geo = f"{province}省{city}市" if province else city
                msg_parts.append(
                    f"（仅供参考）用户的故乡是 {geo}{f' {county}区县' if county else ''}。"
                    f"但如果信中提到的地点不在故乡，请以信中地点为准。"
                )
