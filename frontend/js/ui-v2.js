...
App.navigate = function navigate(page) {
  ...
  const [eyebrow, title, subtitle] = V2_PAGE_META[page] || V2_PAGE_META.game;
...
App.showPostcardDetail = function showPostcardDetail(pc) {
  ...
  overlay.className = 'modal v2-modal';
...
function renderGame() {
  ...
  window._v2PinnedPostcards = pinned;
  const latestEvent = postcards[0] || letters[0] || memories[0];
  el.innerHTML = `
    <div class="game-grid">
      <section class="cork-panel game-board" aria-labelledby="board-title">
        ...
        ${pinned.length ? `<div class="pinned-grid">${pinned.map((pc, index) => `
          <button class="pinned-card tilt-${index + 1}" onclick="App.showPostcardDetail(window._v2PinnedPostcards[${index}])">
          ...
          <div class="visual-empty">
            <img src="assets/ui-v2/empty-mailbox-card.webp" alt="暖灯、邮箱和空白信纸">
  ...
  el.innerHTML = `
    <div class="write-grid">
      <section class="write-stage paper-panel">
        ...
        <div class="env-mailbox" id="env-mailbox"><img src="assets/env_mailbox.png" alt="邮箱"></div>
      </section>
...
let _v2PostcardQuery = '';
function filterPostcards(value) {
  _v2PostcardQuery = value;
  renderPostcards();
...
function renderPostcards() {
  ...
  const query = _v2PostcardQuery.trim().toLowerCase();
  ...
  window._v2FilteredPostcards = filtered;
  el.innerHTML = `
    <section class="album-toolbar paper-panel">
      ...
      <label class="search-box"><span>⌕</span><input class="inp" id="pc-filter" value="${App._e(_v2PostcardQuery)}" placeholder="搜索地点、情绪或标题" oninput="filterPostcards(this.value)"></label>
    </section>
    ${filtered.length ? `<div class="postcard-wall">${filtered.map((pc, index) => `
      <button class="album-card" onclick="App.showPostcardDetail(window._v2FilteredPostcards[${index}])">
      ...
      <div class="visual-empty wide-empty"><img src="assets/ui-v2/empty-mailbox-card.webp" alt="空白信纸与暖灯"><div><h3>${query ? '没有找到这段风景' : '相册墙还是空的'}</h3><p>${query ? '换一个地点或情绪试试。' : '寄出一封信，第一张明信片就会被钉在这里。'}</p>${query ? '<button class="btn btn-sec" onclick="filterPostcards(\'\')">清除搜索</button>' : '<button class="btn btn-pri" onclick="App.navigate(\'write_letter\')">去写信</button>'}</div></div>`}`;
}
    ...
    <div class="memory-grid">
      <section class="memory-list-panel paper-panel">
        ...
        ${memories.length ? `<div class="memory-list">${memories.map((memory, index) => `
          <button class="memory-note" onclick="showMemDetail(window._v2Memories[${index}])">
          ...
          </button>`).join('')}</div>` : `<div class="visual-empty memory-empty"><img src="assets/ui-v2/empty-mailbox-card.webp" alt="空白手账与暖灯"><div><h3>第一页还没有落笔</h3><p>记下一个气味、一阵风，或者某个突然想起的人。</p><button class="btn btn-pri" onclick="showMemForm()">写下第一条记忆</button></div></div>`}
      </section>
...
const _v2OriginalSaveHome = saveHome;
saveHome = async function saveHomeV2() {
  await _v2OriginalSaveHome();
  App.syncShell();
};

const _v2OriginalNextDay = nextDay;
nextDay = async function nextDayV2() {
  await _v2OriginalNextDay();
  App.syncShell();
};
