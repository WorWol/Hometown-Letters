...
function _postcardResultsHtml(filtered, query) {
  ...
  return filtered.length ? `<div class="postcard-wall">${filtered.map((pc, index) => `
      <button class="album-card" onclick="App.showPostcardDetail(window._postcardWall[${index}])">
        ...
        <span class="album-copy"><small>${App._e(pc.place || '沿途')}${pc.mood ? ` · ${App._e(pc.mood)}` : ''}</small><strong>${App._e(pc.title || '无题明信片')}</strong><em>${pc.generationPlace ? `画面取景：${App._e(pc.generationPlace)}` : ''}${pc.createdAt ? ` · ${new Date(pc.createdAt).toLocaleDateString('zh-CN')}` : ''}</em></span>
      </button>`).join('')}</div>` : `
      <div class="visual-empty large-empty paper-panel"><img src="assets/workbench/empty-mailbox-card.webp" alt="空白相册桌面"><div><h3>${normalized ? '没有找到符合条件的明信片' : '还没有明信片'}</h3><p>${normalized ? '请尝试其他地点、标签或情绪。' : '写完并投递一封信后，生成的明信片会保存在这里。'}</p>${normalized ? '' : '<button class="btn btn-pri" onclick="App.navigate(\'write_letter\')">写第一封信</button>'}</div></div>`;
}
...
function filterPostcards(value) {
  ...
  if (count) count.textContent = `${filtered.length} 张明信片`;
...
function renderPostcards() {
  ...
  el.innerHTML = `
    <section class="album-toolbar paper-panel">
      <div><span class="section-kicker">YOUR POSTCARD ALBUM</span><h2 id="pc-result-count">${filtered.length} 张明信片</h2><p>可以按地点、标签或情绪搜索。</p></div>
